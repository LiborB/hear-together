...
export function Button(props: ButtonProps) {
    ...
    return !outlined ? (
        <button {...slicedProps} className={`bg-teal-400 focus:outline-none text-black px-4 py-1 hover:bg-teal-300 ${slicedProps.className}`}></button>
    ) : (
        <button
            {...slicedProps}
...
export function FormError(props: FormErrorProps) {
    return <span {...props} className={`text-red-500 text-xs px-3 ${props.className}`}></span>;
}
