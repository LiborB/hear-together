    ...
    <div className="w-full">
      <div className="relative">
        ...
        {Boolean(foundSongs.length) && isComponentVisible && (
          <ul
            ref={ref}
            className="absolute z-10 transform translate-y-1 rounded-md border border-teal-100 bg-gray-800 w-full"
          >
            {foundSongs.map((item) => (
              <li
                onClick={() => handleSearchItemClick(item)}
                className="text-gray-200 px-2 flex justify-between w-full align-middle pb-2 cursor-pointer hover:opacity-50"
                key={item.id}
              >
