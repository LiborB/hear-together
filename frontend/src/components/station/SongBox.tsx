  ...
  return (
    <div className="song-box-container">
      <div>
        ...
        <div className="px-2">
          <div className="flex justify-between text-teal-300 pt-4 items-center pb-4 border-b border-gray-600">
            {songPlaying?.id > 0 ? (
              <Fragment>
                <div className="flex items-center">
                  <span className="pr-2">{songPlaying?.title}</span>
                  <div className="loader mt-1" />
                </div>

                {props.isUserOwner && (
                  <button className="skip-button" onClick={skipClick}>
                    Skip
                  </button>
                )}
              </Fragment>
          ...
          <div className="pt-4 pb-2 text-xl">Up Next</div>
          {songsInQueue.map((song) => (
            ...
            >
              ...
              <Moment utc date={song.duration * 1000} format="mm:ss" />
            </div>
          ))}
        </div>
      </div>
